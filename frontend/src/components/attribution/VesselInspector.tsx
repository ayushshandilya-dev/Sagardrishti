    ...
    <div className="rounded-panel border border-line bg-bg-1 px-3.5 py-3">
      <div className="mb-2 flex items-center justify-between border-b border-line pb-2">
        <span className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-dim">
          <Ship className="h-3.5 w-3.5 text-aqua" />
          Vessel attribution
        </span>
        <Link
          href={`/vessels/${v.imo}`}
      ...
      <div className="flex items-baseline gap-2">
        <span className="text-sm font-semibold text-ink">{v.vesselName}</span>
      ...
      <div className="mt-2.5 flex flex-col gap-1">
        {v.reasons.map((r, i) => (
          ...
          >
            ...
            />
            <span className="text-[10px] leading-snug text-ink-dim">{r}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mt-2.5 flex items-center gap-2 rounded-panel border border-red/25 bg-red/5 px-2.5 py-2">
        ...
        <span className="text-[10px] leading-snug text-ink-dim">{v.enforcementAction}</span>
      </div>

      {v.historicalIncidents && v.historicalIncidents.length > 0 && (
        <div className="mt-2.5">
          <div className="meta-label mb-1 flex items-center gap-1">
            <History className="h-3 w-3 text-amber" />
            Prior incidents
          </div>
          <div className="flex flex-col gap-1">
            {v.historicalIncidents.slice(0, 2).map((h, i) => (
              <div key={i} className="flex items-center justify-between font-mono text-[9px] text-ink-dim">
