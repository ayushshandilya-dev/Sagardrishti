...
4. **Kinematic Fusion & Bayesian Attribution**: Funnels candidate vessel tracks through a 4D cylinder gate ($R \le 35\text{ km}, |\Delta t| \le 6\text{ h}$), flags dark-ship AIS silences ($>1.0\text{ hour}$) and loitering slowdowns, and scores suspects using a reconciled Gaussian error kernel ($\sigma_{\text{origin}} = 230.7\text{ m}, \sigma_{\text{registration}} = 35.4\text{ m} \implies \sigma_{\text{kernel}} \approx 233.4\text{ m}$) under a **Dirichlet-Categorical conjugate prior**.
...
* **Dark-Ship Gap Detection**: Flags intentional AIS transponder silences ($>1.0\text{ hour}$) and loitering slowdowns during nighttime windows.
